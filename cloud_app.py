...
def get_nx_product_detail(df: pd.DataFrame, store_code: str, category_name: str, target_date):
    mask = (
        (df[NX_COL_STORE].astype(str) == str(store_code))
        & (df[NX_COL_NGANH_PHAN_TICH] == category_name)
        & (df[NX_COL_DATE].dt.date == target_date)
    )
    day_df = df[mask]
    if day_df.empty:
        return []
...
def build_nx_category_bubble(nx_df: pd.DataFrame, store_code: str, category_name: str, target_date=None):
    ...
    rows = get_nx_product_detail(nx_df, store_code, category_name, target_date)
    if not rows:
        return None
    ...
    alt_text = f"NHẬP/XUẤT {category_name} {target_date.strftime('%d/%m/%Y')} - DT: {format_money(total_rev)}"
    return bubble, alt_text


def build_nx_summary_bubble(nx_df: pd.DataFrame, store_code: str, target_date=None):
    ...
    mask_date = mask_store & (nx_df[NX_COL_DATE].dt.date == target_date)
    ...
    alt_text = f"TỔNG QUAN NHẬP/XUẤT {target_date.strftime('%d/%m/%Y')} - DT: {format_money(total_rev)}"
    return bubble, alt_text
