...
def build_fresh_chitiet_bubble(nganh_hang_path: str, phieu_nhap_path, store_code: str):
    nh_df = load_nganh_hang_df(nganh_hang_path)
    ...
    body_contents.append({
        "type": "text",
        "text": "⚠️ Số Hủy / Số Mất Mát: chưa có báo cáo tương ứng từ hệ thống.",
        "size": "xxs",
        "color": COLOR_NEUTRAL,
        "margin": "md",
        "wrap": True,
...
def push_fresh_chitiet(nganh_hang_path: str, phieu_nhap_path):
    result = build_fresh_chitiet_bubble(nganh_hang_path, phieu_nhap_path, STORE_CODE)
    if result is None:
        log("[FRESHCT] Không tìm thấy dữ liệu để tính chi tiết Fresh.")
        return False
    ...
    resp = requests.post(
        f"{CLOUD_URL.rstrip('/')}/update",
        headers={"X-Push-Secret": PUSH_SECRET},
        json={"store_code": STORE_CODE, "category": "FRESHCT", "bubble": bubble, "alt_text": alt_text},
        timeout=20,
    )
    if resp.status_code == 200:
        log(f"[FRESHCT] Đã đẩy báo cáo chi tiết Nhập/Bán Fresh thành công. {alt_text}")
        return True
...
def main():
    ...
    last_mtimes = {"sieu_thi": None, "nganh_hang": None, "chi_tiet": None, "phieu_nhap": None}
    ...
    while True:
        try:
            ...
            freshct_changed = (nh_mtime != last_mtimes["nganh_hang"]) or (pn_mtime != last_mtimes["phieu_nhap"])
            ...
            if freshct_changed and nganh_hang_path:
                log("Phát hiện thay đổi dữ liệu Nhập/Bán - đang tính lại báo cáo FRESHCT...")
                if push_fresh_chitiet(nganh_hang_path, phieu_nhap_path):
                    last_mtimes["nganh_hang"] = nh_mtime
