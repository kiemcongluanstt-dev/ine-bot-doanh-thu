...
def webhook():
    ...
    for event in data.get("events", []):
        ...
        if text.startswith("NAM "):
            ...
            store_code = text[6:].strip()
            _lookup_and_reply(reply_token, f"FRESH_{store_code}", f"ngành hàng Fresh cho mã {store_code}")
        elif text.startswith("FRESHCT "):
            store_code = text[8:].strip()
            _lookup_and_reply(reply_token, f"FRESHCT_{store_code}", f"chi tiết Nhập/Bán Fresh cho mã {store_code}")
