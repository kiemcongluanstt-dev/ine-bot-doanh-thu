...
CL_COL_PRODUCT = "Tên sản phẩm"
CL_COL_QTY = "Số lượng chênh lệch"
CL_COL_VALUE = "Giá trị chênh lệch"
CL_COL_GHI_CHU = "Ghi chú"

FRESH_CATEGORY_SLUG = {
    "Rau Củ Các Loại": "RAUCU",
    "Trái Cây Các Loại": "TRAICAY",
    "Thịt gia cầm gia súc các loại": "THIT",
    "Thủy Hải Sản Các Loại": "THUYHAISAN",
}
CL_GHI_CHU_HUY = "Clear tồn hằng ngày"
CL_GHI_CHU_MAT_MAT = "Clear tồn 14 ngày k nhập/k xuất"

# --- file "BC Nhap Xuat" (gop san Nhap/Xuat/Huy/Mat mat/Doanh thu theo tung san pham) ---
FILE_PATTERN_NHAP_XUAT = ["BC_nhap_xuat*.xlsx", "_BC_nhap_xuat*.xlsx", "BC_Nhap_Xuat*.xlsx"]

NX_COL_DATE = "Ngày"
NX_COL_STORE = "Mã siêu thị"
NX_COL_PRODUCT = "Tên sản phẩm"
NX_COL_NGANH_PHAN_TICH = "Ngành hàng - Phân tích"
NX_COL_NHAP = "SL thực nhập"
NX_COL_XUAT = "SL thực xuất"
NX_COL_HUY = "SL hủy tồn"
NX_COL_MAT_MAT = "SL mất mát kiểm kê"
NX_COL_REVENUE = "Thành tiền phải thu khách hàng (chưa VAT)"

# 9 nhom chi tiet theo "Nganh hang - Phan tich", GIU NGUYEN khong gop lai
NX_CATEGORIES = [
    "Rau Đà Lạt",
    "Rau Địa Phương",
    "Thịt Địa Phương",
...
NX_CATEGORY_SLUG = {
    ...
    "Trứng Các Loại": "TRUNG",
}

COLOR_HEADER_BG_NX = "#5D4037"
...
def _product_detail_row(label: str, nhap: float, ban: float, huy: float, mm_text: str, mm_color: str) -> dict:
    return {
        "type": "box",
        "layout": "vertical",
        "margin": "sm",
        ...
        "contents": [
            {"type": "text", "text": label, "size": "xs", "color": COLOR_LABEL, "weight": "bold", "wrap": True},
            {
                "type": "box",
                "layout": "horizontal",
                "margin": "xs",
                "contents": [
                    {"type": "text", "text": f"Nhập: {nhap:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                    {"type": "text", "text": f"Bán: {ban:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                    {"type": "text", "text": f"Hủy: {huy:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                ],
            },
            {
                "type": "box",
                ...
                "margin": "xs",
                "contents": [
                    {"type": "text", "text": "Mất mát", "size": "xxs", "color": COLOR_NEUTRAL, "flex": 1},
                    {"type": "text", "text": mm_text, "size": "xs", "color": mm_color, "weight": "bold", "align": "end", "flex": 1},
                ],
            },
        ],
    }
...
def get_product_level_detail(store_code: str, category_full_name: str, target_date, ban_df_all, nhap_df_all, cl_df_all):
    """Tra ve list (ten_sp, so_nhap, so_ban, so_huy, tien_mat_mat) cho 1 nganh hang, 1 ngay cu the."""
    mask_ban = (
        (ban_df_all[CT_COL_STORE].astype(str) == str(store_code))
        & (ban_df_all[CT_COL_DATE].dt.date == target_date)
        & (ban_df_all["Ngành hàng"].astype(str).str.contains(category_full_name, case=False, na=False))
    )
    ban_qty = ban_df_all[mask_ban].groupby(CT_COL_PRODUCT)[CT_COL_QTY].sum()

    nhap_qty = pd.Series(dtype=float)
    if nhap_df_all is not None:
        mask_nhap = (
            (nhap_df_all[PN_COL_STORE].astype(str) == str(store_code))
            & (nhap_df_all[PN_COL_DATE].dt.date == target_date)
            & (nhap_df_all[PN_COL_NGANH_HANG].astype(str).str.contains(category_full_name, case=False, na=False))
        )
        nhap_qty = nhap_df_all[mask_nhap].groupby("Tên sản phẩm")[PN_COL_QTY].sum()

    huy_qty = pd.Series(dtype=float)
    mat_mat_value = pd.Series(dtype=float)
    if cl_df_all is not None:
        mask_cl = (
            (cl_df_all[CL_COL_STORE].astype(str) == str(store_code))
            & (cl_df_all[CL_COL_DATE].dt.date == target_date)
            & (cl_df_all[CL_COL_NGANH_HANG].astype(str).str.contains(category_full_name, case=False, na=False))
        )
        cl_day = cl_df_all[mask_cl]
        huy_df = cl_day[cl_day[CL_COL_GHI_CHU] == CL_GHI_CHU_HUY]
        huy_qty = huy_df.groupby(CL_COL_PRODUCT)[CL_COL_QTY].sum()
        mat_mat_value = cl_day.groupby(CL_COL_PRODUCT)[CL_COL_VALUE].sum()

    all_products = set(ban_qty.index) | set(nhap_qty.index) | set(huy_qty.index) | set(mat_mat_value.index)
    rows = []
    for p in all_products:
        n = nhap_qty.get(p, 0)
        b = ban_qty.get(p, 0)
        h = huy_qty.get(p, 0)
        mm = mat_mat_value.get(p, 0)
        if n or b or h or mm:
            rows.append((p, n, b, h, mm))
    rows.sort(key=lambda x: -abs(x[4]))
    return rows


def build_fresh_category_bubble(store_code: str, category_full_name: str, ban_df_all, nhap_df_all, cl_df_all, target_date=None):
    if target_date is None:
        mask_store = ban_df_all[CT_COL_STORE].astype(str) == str(store_code)
        if not mask_store.any():
            return None
        target_date = ban_df_all[mask_store][CT_COL_DATE].max().date()

    rows = get_product_level_detail(store_code, category_full_name, target_date, ban_df_all, nhap_df_all, cl_df_all)
    if not rows:
        return None

    weekday = WEEKDAY_VN[pd.Timestamp(target_date).weekday()]
    total_mm = sum(r[4] for r in rows)

    body_contents = [
        {"type": "text", "text": f"🏪 Mã siêu thị {store_code}", "weight": "bold", "size": "md", "wrap": True},
        {"type": "separator", "margin": "md"},
    ]
    for p, n, b, h, mm in rows:
        label = short_product_name(p, 40)
        mm_text = format_money(mm)
        mm_color = COLOR_DOWN if mm < 0 else (COLOR_UP if mm > 0 else COLOR_NEUTRAL)
        body_contents.append(_product_detail_row(label, n, b, h, mm_text, mm_color))
    ...
    body_contents.append({
        ...
        "margin": "md",
        "contents": [
            {"type": "text", "text": "TỔNG TIỀN MẤT MÁT", "weight": "bold", "size": "md", "flex": 3},
            {"type": "text", "text": format_money(total_mm), "weight": "bold", "size": "lg", "color": (COLOR_DOWN if total_mm < 0 else COLOR_UP), "align": "end", "flex": 2},
        ],
    })
    ...
    bubble = {
        ...
        "header": {
            ...
            "spacing": "sm",
            "contents": [
                {"type": "text", "text": f"📋 MMKK CHI TIẾT - {category_full_name.upper()}", "color": COLOR_WHITE, "weight": "bold", "size": "md", "wrap": True},
    ...
    alt_text = f"MMKK {category_full_name} {target_date.strftime('%d/%m/%Y')} - Mất mát: {format_money(total_mm)}"
    return bubble, alt_text


def push_fresh_categories(chi_tiet_path: str, phieu_nhap_path, chenh_lech_path):
    ban_df_all = load_chi_tiet_df(chi_tiet_path)
    mask_store = ban_df_all[CT_COL_STORE].astype(str) == str(STORE_CODE)
    if not mask_store.any():
        return False
    all_dates = sorted(ban_df_all[mask_store][CT_COL_DATE].dt.date.unique())
    if not all_dates:
        return False
    latest_date = all_dates[-1]
    recent_dates = [d for d in all_dates if (latest_date - d).days < HISTORY_DAYS]

    nhap_df_all = load_phieu_nhap_df(phieu_nhap_path) if phieu_nhap_path else None
    cl_df_all = load_chenh_lech_df(chenh_lech_path) if chenh_lech_path else None

    any_ok = False
    for cat_name, slug in FRESH_CATEGORY_SLUG.items():
        for d in recent_dates:
            result = build_fresh_category_bubble(STORE_CODE, cat_name, ban_df_all, nhap_df_all, cl_df_all, target_date=d)
            if result is None:
                continue
            bubble, alt_text = result
            date_str = d.strftime("%Y-%m-%d")
            resp = _post_update(STORE_CODE, f"FRESHCT_{slug}", bubble, alt_text, date_str=date_str)
            if resp.status_code == 200:
                any_ok = True
            else:
                log(f"[MMKK-{slug}] LỖI đẩy ngày {d.strftime('%d/%m')} ({resp.status_code}): {resp.text}")

        result = build_fresh_category_bubble(STORE_CODE, cat_name, ban_df_all, nhap_df_all, cl_df_all)
        if result is not None:
            bubble, alt_text = result
            resp = _post_update(STORE_CODE, f"FRESHCT_{slug}", bubble, alt_text, date_str=None)
            if resp.status_code == 200:
                any_ok = True
                log(f"[MMKK-{slug}] Đã đẩy chi tiết {cat_name} thành công. {alt_text}")
            else:
                log(f"[MMKK-{slug}] LỖI đẩy dữ liệu ({resp.status_code}): {resp.text}")

    return any_ok


# ============ BÁO CÁO NHẬP/XUẤT CHI TIẾT 9 NHÓM (lệnh NXST) - dùng 1 file gộp sẵn ============

def load_nhap_xuat_df(file_path: str) -> pd.DataFrame:
    df = pd.read_excel(file_path)
    df[NX_COL_DATE] = pd.to_datetime(df[NX_COL_DATE])
    return df


def get_nx_product_detail(df: pd.DataFrame, store_code: str, category_name: str, start_date, end_date=None):
    if end_date is None:
        end_date = start_date
    mask = (
        (df[NX_COL_STORE].astype(str) == str(store_code))
        & (df[NX_COL_NGANH_PHAN_TICH] == category_name)
        & (df[NX_COL_DATE].dt.date >= start_date)
        & (df[NX_COL_DATE].dt.date <= end_date)
    )
    day_df = df[mask]
    if day_df.empty:
        return []
    grouped = day_df.groupby(NX_COL_PRODUCT)[[NX_COL_NHAP, NX_COL_XUAT, NX_COL_HUY, NX_COL_MAT_MAT, NX_COL_REVENUE]].sum()
    rows = []
    for p, r in grouped.iterrows():
        n, x, h, mm, rev = r[NX_COL_NHAP], r[NX_COL_XUAT], r[NX_COL_HUY], r[NX_COL_MAT_MAT], r[NX_COL_REVENUE]
        if n or x or h or mm or rev:
            rows.append((p, n, x, h, mm, rev))
    rows.sort(key=lambda x: -x[5])
    return rows


def _nx_product_row(label: str, nhap: float, xuat: float, huy: float, mat_mat: float, revenue: float) -> dict:
    return {
        "type": "box",
        "layout": "vertical",
        "margin": "sm",
        ...
        "contents": [
            {"type": "text", "text": label, "size": "xs", "color": COLOR_LABEL, "weight": "bold", "wrap": True},
            {
                ...
                "contents": [
                    {"type": "text", "text": f"Nhập: {nhap:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                    {"type": "text", "text": f"Xuất: {xuat:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                ],
            },
            {
                "type": "box",
                ...
                "contents": [
                    {"type": "text", "text": f"Hủy: {huy:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                    {"type": "text", "text": f"Mất mát: {mat_mat:.0f}", "size": "xxs", "color": "#111111", "flex": 1},
                ],
            },
            {
                "type": "box",
                ...
                "margin": "xs",
                "contents": [
                    {"type": "text", "text": "Doanh thu", "size": "xxs", "color": COLOR_NEUTRAL, "flex": 1},
                    {"type": "text", "text": format_money(revenue), "size": "xs", "color": COLOR_TOTAL, "weight": "bold", "align": "end", "flex": 1},
                ],
            },
        ],
    }
...
def build_nx_category_bubble(nx_df: pd.DataFrame, store_code: str, category_name: str, target_date=None, end_date=None, period_label=None):
    mask_store_cat = (nx_df[NX_COL_STORE].astype(str) == str(store_code)) & (nx_df[NX_COL_NGANH_PHAN_TICH] == category_name)
    if not mask_store_cat.any():
        return None
    if target_date is None:
        target_date = nx_df[mask_store_cat][NX_COL_DATE].max().date()
    if end_date is None:
        end_date = target_date

    rows = get_nx_product_detail(nx_df, store_code, category_name, target_date, end_date)
    if not rows:
        return None

    if period_label is None:
        weekday = WEEKDAY_VN[pd.Timestamp(target_date).weekday()]
        period_label = f"{weekday}, {target_date.strftime('%d/%m/%Y')}"
    total_nhap = sum(r[1] for r in rows)
    total_xuat = sum(r[2] for r in rows)
    total_huy = sum(r[3] for r in rows)
    total_mm = sum(r[4] for r in rows)
    ...
    for p, n, x, h, mm, rev in rows:
        body_contents.append(_nx_product_row(short_product_name(p, 40), n, x, h, mm, rev))
    ...
    body_contents.append({
        ...
        "margin": "md",
        "contents": [
            {"type": "text", "text": "TỔNG", "weight": "bold", "size": "sm"},
            {
                "type": "box", "layout": "horizontal", "margin": "xs",
                "contents": [
                    {"type": "text", "text": f"Nhập: {total_nhap:.0f}", "size": "xs", "flex": 1},
                    {"type": "text", "text": f"Xuất: {total_xuat:.0f}", "size": "xs", "flex": 1},
                ],
            },
            {
                "type": "box", "layout": "horizontal", "margin": "xs",
                "contents": [
                    {"type": "text", "text": f"Hủy: {total_huy:.0f}", "size": "xs", "flex": 1},
                    {"type": "text", "text": f"Mất mát: {total_mm:.0f}", "size": "xs", "flex": 1},
                ],
            },
            {
                "type": "box", "layout": "horizontal", "margin": "xs",
                "contents": [
                    {"type": "text", "text": "Tổng doanh thu", "size": "sm", "weight": "bold", "flex": 1},
                    {"type": "text", "text": format_money(total_rev), "size": "md", "weight": "bold", "color": COLOR_TOTAL, "align": "end", "flex": 1},
                ],
            },
        ],
    })
    ...
    bubble = {
        ...
        "header": {
            "type": "box",
            "layout": "vertical",
            "backgroundColor": COLOR_HEADER_BG_NX,
            "paddingAll": "20px",
            "spacing": "sm",
            "contents": [
                {"type": "text", "text": f"📦 NHẬP/XUẤT - {category_name.upper()}", "color": COLOR_WHITE, "weight": "bold", "size": "md", "wrap": True},
                {"type": "text", "text": f"📅 {period_label}", "color": COLOR_WHITE, "size": "sm"},
            ],
        },
        "body": {
            "type": "box",
    ...
    alt_text = f"NHẬP/XUẤT {category_name} {period_label} - DT: {format_money(total_rev)}"
    return bubble, alt_text


def build_nx_summary_bubble(nx_df: pd.DataFrame, store_code: str, target_date=None, end_date=None, period_label=None):
    mask_store = nx_df[NX_COL_STORE].astype(str) == str(store_code)
    if not mask_store.any():
        return None
    if target_date is None:
        target_date = nx_df[mask_store][NX_COL_DATE].max().date()
    if end_date is None:
        end_date = target_date

    mask_date = mask_store & (nx_df[NX_COL_DATE].dt.date >= target_date) & (nx_df[NX_COL_DATE].dt.date <= end_date)
    day_df = nx_df[mask_date]
    if day_df.empty:
        return None
    ...
    rows = [["NHÓM HÀNG", "NHẬP", "XUẤT", "DOANH THU"]]
    total_n = total_x = total_rev = 0
    for cat in NX_CATEGORIES:
        cat_df = day_df[day_df[NX_COL_NGANH_PHAN_TICH] == cat]
        n = cat_df[NX_COL_NHAP].sum()
        x = cat_df[NX_COL_XUAT].sum()
        rev = cat_df[NX_COL_REVENUE].sum()
        total_n += n
        total_x += x
        total_rev += rev
        rows.append([short_name(cat, 16), f"{n:.0f}", f"{x:.0f}", format_money_short(rev)])
    rows.append(["TỔNG", f"{total_n:.0f}", f"{total_x:.0f}", format_money_short(total_rev)])

    grid_contents = [_grid_row(r, header=(i == 0)) for i, r in enumerate(rows)]
    ...
    bubble = {
        ...
        "header": {
            ...
            "spacing": "sm",
            "contents": [
                {"type": "text", "text": "📦 TỔNG QUAN NHẬP/XUẤT (9 NHÓM)", "color": COLOR_WHITE, "weight": "bold", "size": "md", "wrap": True},
                {"type": "text", "text": f"📅 {period_label}", "color": COLOR_WHITE, "size": "sm"},
            ],
        },
        "body": {
    ...
    alt_text = f"TỔNG QUAN NHẬP/XUẤT {period_label} - DT: {format_money(total_rev)}"
    return bubble, alt_text


def push_nx_reports(nx_path: str):
    nx_df = load_nhap_xuat_df(nx_path)
    mask_store = nx_df[NX_COL_STORE].astype(str) == str(STORE_CODE)
    if not mask_store.any():
        return False
    all_dates = sorted(nx_df[mask_store][NX_COL_DATE].dt.date.unique())
    if not all_dates:
        return False
    latest_date = all_dates[-1]
    recent_dates = [d for d in all_dates if (latest_date - d).days < HISTORY_DAYS]
    ...
    # bao cao theo tung nhom (9 nhom)
    for cat_name, slug in NX_CATEGORY_SLUG.items():
        for d in recent_dates:
            result = build_nx_category_bubble(nx_df, STORE_CODE, cat_name, target_date=d)
            if result is None:
                continue
            bubble, alt_text = result
            resp = _post_update(STORE_CODE, f"NX_{slug}", bubble, alt_text, date_str=d.strftime("%Y-%m-%d"))
            if resp.status_code == 200:
                any_ok = True
            else:
                log(f"[NX-{slug}] LỖI đẩy ngày {d.strftime('%d/%m')} ({resp.status_code}): {resp.text}")

        result = build_nx_category_bubble(nx_df, STORE_CODE, cat_name)
        if result is not None:
            bubble, alt_text = result
            resp = _post_update(STORE_CODE, f"NX_{slug}", bubble, alt_text, date_str=None)
            if resp.status_code == 200:
                any_ok = True
                log(f"[NX-{slug}] Đã đẩy {cat_name} thành công. {alt_text}")
            else:
                log(f"[NX-{slug}] LỖI đẩy dữ liệu ({resp.status_code}): {resp.text}")

    # bao cao tong quan 9 nhom
    for d in recent_dates:
        result = build_nx_summary_bubble(nx_df, STORE_CODE, target_date=d)
        if result is None:
            continue
        bubble, alt_text = result
        resp = _post_update(STORE_CODE, "NX_SUMMARY", bubble, alt_text, date_str=d.strftime("%Y-%m-%d"))
        if resp.status_code == 200:
            any_ok = True

    result = build_nx_summary_bubble(nx_df, STORE_CODE)
    if result is not None:
        bubble, alt_text = result
        resp = _post_update(STORE_CODE, "NX_SUMMARY", bubble, alt_text, date_str=None)
        if resp.status_code == 200:
            any_ok = True
            log(f"[NX-SUMMARY] Đã đẩy tổng quan 9 nhóm thành công. {alt_text}")
        else:
            log(f"[NX-SUMMARY] LỖI đẩy dữ liệu ({resp.status_code}): {resp.text}")

    # bao cao "THANG NAY" (cong don tu ngay 1 cua thang chua ngay moi nhat, den ngay moi nhat)
    mtd_start = latest_date.replace(day=1)
    mtd_label = f"Từ {mtd_start.strftime('%d/%m')} đến {latest_date.strftime('%d/%m/%Y')} (tháng này)"

    for cat_name, slug in NX_CATEGORY_SLUG.items():
        result = build_nx_category_bubble(nx_df, STORE_CODE, cat_name, target_date=mtd_start, end_date=latest_date, period_label=mtd_label)
        if result is not None:
            bubble, alt_text = result
            resp = _post_update(STORE_CODE, f"NX_{slug}", bubble, alt_text, date_str="MTD")
            if resp.status_code == 200:
                any_ok = True
            else:
                log(f"[NX-{slug}-MTD] LỖI đẩy dữ liệu ({resp.status_code}): {resp.text}")

    result = build_nx_summary_bubble(nx_df, STORE_CODE, target_date=mtd_start, end_date=latest_date, period_label=mtd_label)
    if result is not None:
        bubble, alt_text = result
        resp = _post_update(STORE_CODE, "NX_SUMMARY", bubble, alt_text, date_str="MTD")
        if resp.status_code == 200:
            any_ok = True
            log(f"[NX-SUMMARY-MTD] Đã đẩy tổng quan tháng này thành công. {alt_text}")
        else:
            log(f"[NX-SUMMARY-MTD] LỖI đẩy dữ liệu ({resp.status_code}): {resp.text}")
...
def main():
    ...
    last_mtimes = {
        "sieu_thi": None, "nganh_hang": None, "chi_tiet": None, "phieu_nhap": None, "chenh_lech": None,
        "freshcat_chi_tiet": None, "freshcat_phieu_nhap": None, "freshcat_chenh_lech": None,
        "nhap_xuat": None,
    }
    ...
    while True:
        try:
            ...
            nhap_xuat_path = find_latest_file(FILE_PATTERN_NHAP_XUAT)
            ...
            nx_mtime = os.path.getmtime(nhap_xuat_path) if nhap_xuat_path else None
            ...
            freshcat_changed = (
                (ct_mtime != last_mtimes["freshcat_chi_tiet"])
                or (pn_mtime != last_mtimes["freshcat_phieu_nhap"])
                or (cl_mtime != last_mtimes["freshcat_chenh_lech"])
            )
            nx_changed = nx_mtime != last_mtimes["nhap_xuat"]
            ...
            if freshcat_changed and chi_tiet_path:
                log("Phát hiện thay đổi dữ liệu - đang tính lại chi tiết từng ngành hàng Fresh (MMKKST theo nhóm)...")
                if push_fresh_categories(chi_tiet_path, phieu_nhap_path, chenh_lech_path):
                    last_mtimes["freshcat_chi_tiet"] = ct_mtime
                    last_mtimes["freshcat_phieu_nhap"] = pn_mtime
                    last_mtimes["freshcat_chenh_lech"] = cl_mtime
            ...
            if nx_changed and nhap_xuat_path:
                log("Phát hiện thay đổi file Nhập/Xuất (BC Nhap Xuat) - đang tính lại báo cáo NXST...")
                if push_nx_reports(nhap_xuat_path):
                    last_mtimes["nhap_xuat"] = nx_mtime
